-- Infrastructure/Serialization/JSON.lua — JSON utilities (encode, pretty, decode)
local __p = { ... }
...
local function is_array(t)
  local count = 0
  for k,_ in pairs(t) do if type(k) ~= 'number' then return false end count = count + 1 end
...
local function encode_val(v, stack)
  ...
  elseif tv == 'table' then
    ...
    if arr then
      ...
      local parts = {}
      for _,k in ipairs(keys) do parts[#parts+1] = escape_str(tostring(k))..':'..encode_val(v[k], stack) end
      stack[v] = nil
      return '{'..table.concat(parts, ',')..'}'
    end
  end
...
function JSON.Encode(value) return encode_val(value, {}) end
...
local function encode_pretty(v, stack, depth)
  ...
  if arr then
    ...
    local parts={}
    for _,k in ipairs(keys) do parts[#parts+1] = indent(depth+1)..escape_str(tostring(k))..': '..encode_pretty(v[k], stack, depth+1) end
    stack[v]=nil
    return '{\n'..table.concat(parts, ',\n')..'\n'..indent(depth)..'}'
  end
end

function JSON.EncodePretty(value) return encode_pretty(value, {}, 0) end

-- Decoder (pure Lua; preserves \uXXXX escapes)
do
  ...
  local function parse_value(str, i)
    ...
    elseif c == '-' or c:match('%d') then
      ...
      while j <= #str do
        ...
        elseif ch == '.' and not hasDot then hasDot = true; j = j + 1
        elseif (ch == 'e' or ch == 'E') and not hasExp then hasExp = true; j = j + 1; local sign = string.sub(str, j, j); if sign == '+' or sign == '-' then j = j + 1 end
        else break end
      end
  ...
  local num = tonumber(string.sub(str, ii, jj))
    ...
    elseif string.sub(str, i, i+3) == 'null' then
      return nil, i + 4
    elseif c == '{' then
      local obj = {}; i = i + 1; i = skip_ws(str, i)
  ...
  obj[tostring(key)] = val
    ...
    elseif c == '[' then
      local arr = {}; i = i + 1; i = skip_ws(str, i)
  ...
  function JSON.Decode(str)
    ...
    if i <= #str then return nil, "trailing characters", i end
    return val
  end
...
if Addon.provide then
  ...
  -- New preferred key
  Addon.provide('Serialization.JSON', JSON, { lifetime = 'SingleInstance' })
end
