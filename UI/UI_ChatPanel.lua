...
local function CreateChatPanel(parent)
  ...
  ticon:SetSize(14, 14)
  if ticon.SetAtlas then ticon:SetAtlas('hud-MainMenuBar-dropdownarrow', true) else ticon:SetTexture('Interface/Buttons/UI-MicroStream-Down') end
  ...
  local function openTemplatesMenu()
    ...
    for _,txt in ipairs(items) do
      ...
      y = y - 22
      local nt = b:CreateTexture(nil,'ARTWORK') nt:SetAllPoints(); nt:SetTexture('Interface/Buttons/WHITE8x8'); nt:SetVertexColor(1,1,1,0.06)
      b:SetNormalTexture(nt)
      local ht = b:CreateTexture(nil,'ARTWORK') ht:SetAllPoints(); ht:SetTexture('Interface/Buttons/WHITE8x8'); ht:SetVertexColor(1,1,1,0.12)
      b:SetHighlightTexture(ht)
      local fs = b:CreateFontString(nil,'OVERLAY','GameFontNormalSmall') fs:SetPoint('LEFT', 6, 0); fs:SetJustifyH('LEFT') fs:SetText(txt)
  ...
  local function showCopyBox(text)
    ...
    local br = box:CreateTexture(nil,'BORDER'); br:SetPoint('TOPLEFT',1,-1); br:SetPoint('BOTTOMRIGHT',-1,1); br:SetColorTexture(1,1,1,0.10)
    local eb = CreateFrame('EditBox', nil, box)
    eb:SetPoint('TOPLEFT', box, 'TOPLEFT', 8, -8)
    eb:SetPoint('BOTTOMRIGHT', box, 'BOTTOMRIGHT', -28, 8)
    eb:SetAutoFocus(true)
    eb:SetFont((select(1, _G.GameFontNormal:GetFont())), (select(2, _G.GameFontNormal:GetFont())), (select(3, _G.GameFontNormal:GetFont())))
    ...
    close:SetSize(16,16)
    local nt = close:CreateTexture(nil,'ARTWORK') nt:SetAllPoints(); nt:SetTexture('Interface/Buttons/WHITE8x8'); nt:SetVertexColor(1,1,1,0.10)
    close:SetNormalTexture(nt)
    local ht = close:CreateTexture(nil,'ARTWORK') ht:SetAllPoints(); ht:SetTexture('Interface/Buttons/WHITE8x8'); ht:SetVertexColor(1,1,1,0.18)
    close:SetHighlightTexture(ht)
    local xfs = close:CreateFontString(nil,'OVERLAY','GameFontNormalSmall'); xfs:SetPoint('CENTER'); xfs:SetText('X')
    local function dispose() box:Hide(); box:SetParent(nil) end
  ...
  frame:SetScript('OnShow', function()
    if chat and type(chat.Subscribe) == 'function' and not unsubscribe then
      unsubscribe = chat:Subscribe(function(msg)
        ...
        local actions = ''
        if author ~= '' then
          actions = string.format(' |cff99ddaa[|Hgr:invite:%s|hInvite|h]|r |cffff8899[|Hgr:blacklist:%s|hBlacklist|h]|r |cffddddaa[|Hgr:tpl:%s|hTemplates|h]|r', author, author, author)
        end
        ...
        feed:AddMessage(string.format('|cff88ccff[%02d:%02d]|r |cffaacc88[%s]|r %s: %s%s', hh, mm, chan, author, text, actions))
    ...
    if emptyHint and type(feed.GetNumMessages)=='function' and (feed:GetNumMessages() or 0) == 0 then emptyHint:Show() end
  end)
  frame:SetScript('OnHide', function()
    if type(unsubscribe) == 'function' then unsubscribe(); unsubscribe = nil end
  end)
  ...
  feed:SetScript('OnHyperlinkClick', function(_, link, text, button)
    ...
    elseif ltype == 'blacklist' then
      local pm = (Addon.Get and (Addon.Get('IProspectManager') or Addon.Get('ProspectsManager'))) or (Addon.require and Addon.require('ProspectsManager'))
        ...
        pcall(function()
          if pm and pm.GetAllGuids and pm.GetProspect then
            for _,pg in ipairs(pm:GetAllGuids() or {}) do
              ...
              if p and p.name and p.name:lower() == tostring(who):lower() then guidMatch = pg; break end
            end
          end
        end)
      end
  ...
  if has1 == 1 then return g end
    ...
    if needProspects == 1 then
      pcall(function()
        if pm and pm.GetAllGuids and pm.GetProspect then
          for _,pg in ipairs(pm:GetAllGuids() or {}) do local p=pm:GetProspect(pg); if p and p.name and p.name:lower()==nm:lower() then g=pg; break end end
        end
      end)
    end
    ...
    pcall(function()
      if IsInGuild and IsInGuild() and GetNumGuildMembers and GetGuildRosterInfo then
        ...
        for i=1,(n or 0) do
          ...
          if type(nname)=='string' and nname:match('^([^%-]+)') and (nname:match('^([^%-]+)')==nm or nname==nm) and type(gguid)=='string' and gguid~='' then g=gguid; break end
        end
      end
    end)
    return g
  ...
  feed:SetScript('OnHyperlinkEnter', function(_, link, text)
    local ltype, rest = tostring(link):match('^gr:([^:]+):(.+)$')
    if not ltype then lastCtxWho=nil; lastCtxCopyId=nil; return end
  ...
  feed:SetScript('OnHyperlinkLeave', function() lastCtxWho=nil; lastCtxCopyId=nil end)

  local function openContextMenuAtCursor()
    local m = CreateFrame('Frame', nil, frame)
    m:SetFrameStrata('FULLSCREEN_DIALOG')
    m:SetSize(180, 10)
    local bg = m:CreateTexture(nil,'BACKGROUND') bg:SetAllPoints(); bg:SetColorTexture(0,0,0,0.9)
    local br = m:CreateTexture(nil,'BORDER') br:SetPoint('TOPLEFT',1,-1); br:SetPoint('BOTTOMRIGHT',-1,1); br:SetColorTexture(1,1,1,0.10)
    local y = -6
    local function addItem(label, cb)
      ...
      local ht = b:CreateTexture(nil,'ARTWORK') ht:SetAllPoints(); ht:SetTexture('Interface/Buttons/WHITE8x8'); ht:SetVertexColor(1,1,1,0.12)
      b:SetHighlightTexture(ht)
      local fs = b:CreateFontString(nil,'OVERLAY','GameFontNormalSmall') fs:SetPoint('LEFT', 6, 0); fs:SetText(label)
      b:SetScript('OnClick', function() m:Hide(); m:SetParent(nil); cb() end)
    end
  ...
  if hasWho == 1 then
      ...
      addItem('Blacklist '..lastCtxWho, function()
        ...
        if matched == 1 then pcall(function() if pm and pm.Blacklist then pm:Blacklist(guid, 'chat') end end)
        else local Main=Addon.Get and Addon.Get('UI.Main') or (Addon.require and Addon.require('UI.Main')); if Main and Main.Show then Main:Show() end; if Main and Main.SelectCategoryByKey then Main:SelectCategoryByKey('blacklist') end end
      end)
