...
local _, Addon = ...
-- Avoid early container build; rely on Addon.List if present
...
if Addon.safeProvide then
  Addon.safeProvide("AccordionSections", AccordionSections, { lifetime = "SingleInstance" })
  Addon.safeProvide("Collections.AccordionSections", AccordionSections, { lifetime = "SingleInstance" })
elseif Addon.provide then
  if not (Addon.IsProvided and Addon.IsProvided("AccordionSections")) then Addon.provide("AccordionSections", AccordionSections, { lifetime = "SingleInstance" }) end
  if not (Addon.IsProvided and Addon.IsProvided("Collections.AccordionSections")) then Addon.provide("Collections.AccordionSections", AccordionSections, { lifetime = "SingleInstance" }) end
end
