...
-- Avoid early Addon.require which would prematurely build DI container; List may be attached later.
local List = Addon.List -- optional for ToList()
...
if Addon.safeProvide then
  Addon.safeProvide("Queue", Queue, { lifetime = "SingleInstance" })
  Addon.safeProvide("Collections.Queue", Queue, { lifetime = "SingleInstance" })
elseif Addon.provide then
  if not (Addon.IsProvided and Addon.IsProvided("Queue")) then Addon.provide("Queue", Queue, { lifetime = "SingleInstance" }) end
  if not (Addon.IsProvided and Addon.IsProvided("Collections.Queue")) then Addon.provide("Collections.Queue", Queue, { lifetime = "SingleInstance" }) end
end
return Queue
