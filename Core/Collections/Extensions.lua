...
local function EnableTableExtensions()
    ...
    -- debug library not guaranteed in WoW sandbox; skip global metatable mutation when unavailable
end
...
if Addon.safeProvide then
    Addon.safeProvide("TableExtensions", Extensions, { lifetime = "SingleInstance" })
    Addon.safeProvide("Collections.Extensions", Extensions, { lifetime = "SingleInstance" })
elseif Addon.provide then
    if not (Addon.IsProvided and Addon.IsProvided("TableExtensions")) then Addon.provide("TableExtensions", Extensions, { lifetime = "SingleInstance" }) end
    if not (Addon.IsProvided and Addon.IsProvided("Collections.Extensions")) then Addon.provide("Collections.Extensions", Extensions, { lifetime = "SingleInstance" }) end
end
